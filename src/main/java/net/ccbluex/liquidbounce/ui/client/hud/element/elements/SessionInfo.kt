...
@ElementInfo(name = "SessionInfo") class SessionInfo(x: Double = 15.0, y: Double = 10.0, scale: Float = 1F, side: Side = Side(Side.Horizontal.LEFT, Side.Vertical.UP)) : Element(x, y, scale, side) {
    ...
    override fun drawElement(): Border? {
        ...
        fontRenderer.drawStringWithShadow("turi ip ip", x2.toFloat() / 4f, 3f, Color.WHITE.rgb)
        fontRenderer.drawStringWithShadow("Da Choi: $time", 2f, fontRenderer.FONT_HEIGHT + 8f, Color.WHITE.rgb)
        fontRenderer.drawStringWithShadow("Da Giet: " + KillAura.CombatListener.killCounts,2f , fontRenderer.FONT_HEIGHT * 2 + 8f, Color.WHITE.rgb)

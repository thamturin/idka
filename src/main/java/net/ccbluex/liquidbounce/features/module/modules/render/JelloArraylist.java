...
@ModuleInfo(name = "JelloArraylist", description = "skidder", category = ModuleCategory.RENDER)
public class JelloArraylist extends Module {
    ...
    public void renderArraylist() { 
        ...
        for (Module module : modules) {
            ...
            GlStateManager.pushMatrix();
            GL11.glColor4f(1F, 1F, 1F, (module.getAnimation() / 20F) * 0.7f);
            //GlStateManager.resetColor();
            if (useTrueFont.get()) {
                GlStateManager.disableAlpha();
            }
            Fonts.fontSFUI40.drawString(module.getName(), xStart, yStart + 7.5f, -1);
            if (useTrueFont.get()) {
                GlStateManager.enableAlpha();
            }
            GlStateManager.popMatrix();

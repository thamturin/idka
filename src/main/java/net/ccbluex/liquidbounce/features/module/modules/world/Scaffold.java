...
public class Scaffold extends Module {
    ...
    public final ListValue rotationModeValue = new ListValue("RotationMode", new String[]{"Normal", "AAC", "Static", "Static2", "Static3", "Spin", "Grim", "Custom"}, "Normal"); // searching reason
    ...
    public void onUpdate(final UpdateEvent event) {
        if ((!rotationsValue.get() || noHitCheckValue.get() || faceBlock) && placeModeValue.get().equalsIgnoreCase("Legit") && !towerActivation()) {
            place(false);
        }
                ...
                for (double ySearch = 0.1D; ySearch < 0.9D; ySearch += 0.1D) {
                    for (double zSearch = 0.1D; zSearch < 0.9D; zSearch += 0.1D) {
                        ...
                        for (int i = 0; i < (staticYawMode ? 2 : 1); i++) {
                            ...
                            if (keepRotOnJumpValue.get() || !mc.gameSettings.keyBindJump.isKeyDown()) {
                                ...
                                if ((rotationModeValue.get().equalsIgnoreCase("static2") || rotationModeValue.get().equalsIgnoreCase("static3")))
                                    rotation = new Rotation(rotation.getYaw(), staticPitchValue.get());

                                if (rotationModeValue.get().equalsIgnoreCase("custom")) 
                                    rotation = new Rotation(mc.thePlayer.rotationYaw + customYawValue.get(), customPitchValue.get());

                                if (rotationModeValue.get().equalsIgnoreCase("spin") && speenRotation != null) 
                                    rotation = speenRotation;

                                if (rotationModeValue.get().equalsIgnoreCase("grim"))
                                    rotation = new Rotation(mc.thePlayer.rotationYaw + ((mc.thePlayer.movementInput.moveForward < 0) ? 0: 180), placeRotation.getRotation().getPitch());
                            }
